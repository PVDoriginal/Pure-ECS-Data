...
pub struct MinusS(f32);

impl Node<1, 1, 0, 1> for MinusS {
...
pub struct MinusSConfig;

impl AudioNode for MinusS {
    type Configuration = MinusSConfig;
...
impl AudioNodeProcessor for Processor {
    fn process(
        ...
        for patch in events.drain_patches::<MinusS>() {
            let MinusSPatch::Field0(multi) = &patch;
            self.multi = *multi;
